...
window.loginRequired = function (cb) {
    api.get(rootPath + '/api/a/login', {}, cb, function () {
...
window.logout = function () {
    api.post(rootPath + '/api/a/logout', {}, null, dftFail, function () {
        window.location.href = '/a/login?' + $.param({
            next: window.location.pathname
        });
    });
...
window.api.form = function (form, success, fail, before, complete) {
    ...
    form.submit(function () {
        ...
        api.post(rootPath + $(this).attr('action'), data, success, fail, complete);
        return false;
    });
};
